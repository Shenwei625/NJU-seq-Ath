...
if [ $1 == raw_output ]; then
    pigz -dcf ${1}/${PREFIX}/rrna.raw.sam.gz | grep -v "@" | tsv-filter --regex '6:^[0-9]+=$' | tsv-filter --str-eq 7:= |cut -f 1 | uniq > ${1}/${PREFIX}/rrna.lst
    pigz -dcf ${1}/${PREFIX}/mrna.raw.sam.gz | 
        parallel --pipe --block 100M --no-run-if-empty --linebuffer --keep-order -j 24 "
            grep -v '@' | tsv-filter --regex '6:^[0-9]+=$' | tsv-filter --str-eq 7:= | cut -f 1 | uniq
        " > ${1}/${PREFIX}/mrna.lst
    ...
    echo -e "Reads ratio that exactly matches the two:${RATE}%" > ${1}/${PREFIX}/common.tsv
else
    pigz -dcf ${1}/${PREFIX}/${3}/rrna_align.sam.gz | grep -v "@" | tsv-filter --regex '6:^[0-9]+=$' | tsv-filter --str-eq 7:= | cut -f 1 | uniq > ${1}/${PREFIX}/${3}/rrna.lst
    ...
    RATE=$(echo -e "scale=2;(${COMMOM_READS} * 100 / ${TOTAL_READS})" | bc)
    echo -e "Reads ratio that exactly matches the two:${RATE}%" > ${1}/${PREFIX}/${3}/common.tsv
fi
